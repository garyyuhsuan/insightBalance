...
const dateLastyear = new Date(dateYesterday);
dateLastyear.setMonth(dateLastyear.getMonth() - 6);

console.log(dateToday.toISOString().split('T')[0]);
console.log(dateYesterday.toISOString().split('T')[0]);
console.log(dateLastyear.toISOString().split('T')[0]);

inputStart.value = dateLastyear.toISOString().split('T')[0];
inputEnd.value = dateYesterday.toISOString().split('T')[0];
...
function isNANOrNot(x) {
    if (isNaN(Date.parse(x))) {
        ...
        daySelector.select(now.getDate() - 1);
    } else {
        yearSelector.select(new Date(Date.parse(x)).getFullYear());
        monthSelector.select(new Date(Date.parse(x)).getMonth() + 1);
        setTimeout(() => daySelector.select(new Date(Date.parse(x)).getDate()), 0);
    };
}
...
function inputFocus(x) {
    warningText.textContent = "";
    x.focusVisible = true;
    scrollBlock.style.height = "inherit";
    btnWrap.style.height = "0px";
    x.className = 'filter__date';//input取消warning模式
...
function btnOKText(x) {
    ...
    if (isNaN(Date.parse(inputStart.value)) || isNaN(Date.parse(inputEnd.value))) {
        warningText.textContent = "請輸入日期";
        popup_warning = true;//避免popup關閉

        if(isNaN(Date.parse(inputStart.value)) ){
            inputStart.className = 'filter__date error';//input套用warning模式
        }

        if(isNaN(Date.parse(inputEnd.value))){
            inputEnd.className = 'filter__date error';//input套用warning模式
        }
        ...
        inputStart.className = 'filter__date error';
        inputEnd.className = 'filter__date error';
        warningText.textContent = "開始日期不能晚於結束日期";
        popup_warning = true;
        setTimeout(() => {
            popup_warning = false;
        }, 100);
        ...
        btnWrap.style.height = "inherit";
        inputStart.DateOutside = inputStart.value;
        inputEnd.DateOutside = inputEnd.value;
        console.log("取得"+inputStart.DateOutside+"到"+inputEnd.DateOutside+"的資料");
    };
...
class IosSelector {
    constructor(options) {
        ...
        this.itemHeight = 200 * 3 / this.options.count; // 每项高度
    ...
    _touchend(e, touchData) {
        ...
        if (touchData.yArr.length === 1) {
            v = 0;
        }else if(touchData.yArr.length === 0){
            v = 0;
        }else {
    ...
    `;
        ...
        if (this.type === 'infinite') {
            this.elems.highlightList.style.top = -this.itemHeight-20 + 'px';
        }
